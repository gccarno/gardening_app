...
export function useClonePlant() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ entryId, name }: { entryId: number; name: string }) => clonePlant(entryId, name),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['library'] }),
  });
}

export function usePatchLibraryEntry(entryId: number) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (fields: Record<string, unknown>) => patchLibraryEntry(entryId, fields),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['library', entryId] }),
  });
}
